...
cd /d "%~dp0"
title Ruckus All Downloader
chcp 949 >nul
set PYTHONDONTWRITEBYTECODE=1

echo.
echo ============================================================
echo   Ruckus All Downloader
echo ============================================================
echo.
echo Folder: %CD%
echo.

if exist "%~dp0dist\Ruckus_All_Downloader.exe" (
    echo [*] 빌드된 EXE를 실행합니다.
    start "" "%~dp0dist\Ruckus_All_Downloader.exe"
    exit /b 0
)
...
if not exist "%~dp0Ruckus_All_Downloader.py" (
    echo [오류] Ruckus_All_Downloader.py 가 없습니다.
    pause
    exit /b 1
)
...
"%PY%" -B "%~dp0Ruckus_All_Downloader.py"
